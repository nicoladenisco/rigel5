...
public class SanityTorqueUtils2 extends SanityDatabaseUtils
{
     ...
     throws Exception
  {
    ...
    for(TableMap tm : arMaps)
      processTable(new TableMapHelper(tm), dbMap);
  ...
  private void processTable(TableMapHelper tm, DatabaseMap dbMap)
  {
    ...
    try
    {
      processTable0(nomeTabella, tm, dbMap);
  ...
  private void processTable0(String nomeTabella, TableMapHelper tm, DatabaseMap dbMap)
     throws Exception
  {
    ...
    for(int i = 0; i < cs.length; i++)
    {
      ...
      {
        processTable(new TableMapHelper(fk.getForeignTable()), dbMap);
      }
    }

    // inserisce o aggiorna record 0
    if(doInsert(tm.getFullyQualifiedTableName(), crInsert) == 0)
      doUpdate(tm.getFullyQualifiedTableName(), crSelect, crUpdate);
  }

  private static int doInsert(String fullTableName, ColumnValues criteria)
  {
    Connection con = null;
    ...
    try
    {
      ...
      int rv = DbUtils.doInsert(fullTableName, criteria, con);
  ...
  private static int doUpdate(String fullTableName, Criteria selectCriteria, ColumnValues updateValues)
  {
    Connection con = null;
    ...
    {
      con = Transaction.begin();
      int rv = DbUtils.doUpdate(fullTableName, selectCriteria, updateValues, con);
      Transaction.commit(con);
      return rv;
    }
    catch(Exception e)
