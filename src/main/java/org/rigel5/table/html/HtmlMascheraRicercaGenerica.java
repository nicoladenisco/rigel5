 ...
 * </p>
 * <p>
 * Questa classe e' comune alle sue due figlie utilizzate per la ricerca:
 * PeerMascheraRicercaGenerica per le liste che usano oggetti Torque.
 * SqlMascheraRicercaGenerica per le liste che usano query libere.
 * </p>
...
public class HtmlMascheraRicercaGenerica implements MascheraRicercaGenerica
{
  ...
  protected void getComboOrdinamento(StringBuilder sb, int defVal)
  {
    ...
    {
      int i = k + 1;
      sb.append(HtmlUtils.generaOptionCombo(i, i + "-Ascendente", defVal));
    }

    for(int k = 0; k < 4; k++)
    {
      int j = k + 1001;
      sb.append(HtmlUtils.generaOptionCombo(j, j + "-Discendente", defVal));
    }
