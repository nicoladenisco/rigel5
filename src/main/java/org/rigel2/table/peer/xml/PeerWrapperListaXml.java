...
import org.rigel2.table.peer.html.HtmlPeerWrapperBase;
...
public class PeerWrapperListaXml extends HtmlPeerWrapperBase
{
  ...
  public PeerWrapperListaXml()
     throws Exception
  {
    ptm = new org.rigel2.table.peer.xml.PeerTableModel();
  }

  public void init()
     throws Exception
  {
    ((org.rigel2.table.peer.xml.PeerTableModel) ptm).attach(xtbl);

    // imposta numero per pagina
    if(getNumPerPage() != 0)
      limit = getNumPerPage();

    ((org.rigel2.table.peer.xml.PeerTableModel) ptm).init(getObjectClass().newInstance());
  }
     ...
     throws java.lang.Exception
  {
    ...
    if(limit == 0)
    {
      ...
      do
      {
        ...
        recStart += data.size();
        ((org.rigel2.table.peer.xml.PeerTableModel) ptm).rebind(data);
        xtbl.doRows(out);
      }
      while(contFetch);
    ...
    {
      ((org.rigel2.table.peer.xml.PeerTableModel) ptm).rebind(getRecords(new Criteria()));
      xtbl.doXml(out);
    }
  }
